...
// const { parse } = require("path");
    ...
    var getLineInfo = function (input, pos) {
    ...
    function readToken(forceRegexp) {
        ...
        if (puncChars.test(ch)) {
            ++tokPos;
            return finishToken(puncTypes[ch], null);
        }
        if (digit.test(ch)) {
            var nextCh = input.charAt(tokPos + 1);
    ...
    function readWord1(startSpecial) {
        ...
        for (;;) {
            var ch = input.charAt(tokPos);
            if (!escaped) {
                if (ch === "\\") {} else if ((ch <= "\xff" ? identifier : nonASCIIidentifier).test(ch)) word += ch;
                else break;
                ++tokPos;
            } else {
